...
def enroll():
    ...
    # course = Course_Enroll.query.filter(Courses.id == course_id).all()
    course = view_course_details(course_id)
    print(type(course))
    ...
    student = Students.query.filter(Students.id == student_id).all()
    if not(len(student)):
